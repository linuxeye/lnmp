...
src_url=$DOWN_ADDR/mariadb-10.0.19/bintar-${GLIBC_FLAG}-$SYS_BIT_a/mariadb-10.0.19-${GLIBC_FLAG}-${SYS_BIT_b}.tar.gz && Download_src

useradd -M -s /sbin/nologin mysql
mkdir -p $mariadb_data_dir;chown mysql.mysql -R $mariadb_data_dir
tar zxf mariadb-10.0.19-${GLIBC_FLAG}-${SYS_BIT_b}.tar.gz 
mv mariadb-10.0.19-linux-${SYS_BIT_b} $mariadb_install_dir 
