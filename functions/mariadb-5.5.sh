...
src_url=$DOWN_ADDR/mariadb-5.5.43/bintar-${GLIBC_FLAG}-$SYS_BIT_a/mariadb-5.5.43-${GLIBC_FLAG}-${SYS_BIT_b}.tar.gz && Download_src

useradd -M -s /sbin/nologin mysql
mkdir -p $mariadb_data_dir;chown mysql.mysql -R $mariadb_data_dir
tar zxf mariadb-5.5.43-${GLIBC_FLAG}-${SYS_BIT_b}.tar.gz 
mv mariadb-5.5.43-linux-${SYS_BIT_b} $mariadb_install_dir 
