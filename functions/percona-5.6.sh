...
src_url=http://www.percona.com/redir/downloads/Percona-Server-5.6/LATEST/source/tarball/percona-server-5.6.24-72.2.tar.gz && Download_src

useradd -M -s /sbin/nologin mysql
mkdir -p $percona_data_dir;chown mysql.mysql -R $percona_data_dir
tar zxf percona-server-5.6.24-72.2.tar.gz 
cd percona-server-5.6.24-72.2 
...
OS_command
cd ..
[ -d "$percona_install_dir" ] && /bin/rm -rf percona-server-5.6.24-72.2
cd ..
