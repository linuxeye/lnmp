...
src_url=http://www.php.net/distributions/php-5.5.12.tar.gz && Download_src
...
patch -d php-5.5.12 -p0 < fpm-race-condition.patch
cd php-5.5.12
make clean
...
--with-apxs2=$apache_install_dir/bin/apxs $PHP_cache_tmp $PHP_MySQL_options \
...
CFLAGS= CXXFLAGS= ./configure --prefix=$php_install_dir --with-config-file-path=$php_install_dir/etc \
--with-fpm-user=www --with-fpm-group=www --enable-fpm $PHP_cache_tmp $PHP_MySQL_options \
--with-iconv-dir=/usr/local --with-freetype-dir --with-jpeg-dir --with-png-dir --with-zlib \
--with-libxml-dir=/usr --enable-xml --disable-rpath --enable-bcmath --enable-shmop --enable-exif \
--enable-sysvsem --enable-inline-optimization --with-curl --enable-mbregex \
--enable-mbstring --with-mcrypt --with-gd --enable-gd-native-ttf --with-openssl \
