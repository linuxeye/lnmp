...
Install_Percona84() {
  ...
  if [ "${dbinstallmethod}" == "1" ]; then
    tar xzf ./Percona-Server-${percona84_ver}-Linux.x86_64.glibc2.35.tar.gz
    mv Percona-Server-${percona84_ver}-Linux.x86_64.glibc2.35/* ${percona_install_dir}
    #sed -i 's@executing mysqld_safe@executing mysqld_safe\nexport LD_PRELOAD=/usr/local/lib/libjemalloc.so@' ${percona_install_dir}/bin/mysqld_safe
    sed -i "s@/usr/local/Percona-Server-${percona84_ver}-Linux.x86_64.glibc2.35@${percona_install_dir}@g" ${percona_install_dir}/bin/mysqld_safe
  elif [ "${dbinstallmethod}" == "2" ]; then
    boostVersion2=$(echo ${boost_percona_ver} | awk -F. '{print $1"_"$2"_"$3}')
    tar xzf boost_${boostVersion2}.tar.gz
    tar xzf percona-server-${percona84_ver}.tar.gz
    pushd percona-server-${percona84_ver}
  ...
  if [ -d "${percona_install_dir}/support-files" ]; then
    ...
    if [ "${dbinstallmethod}" == "1" ]; then
      rm -rf Percona-Server-${percona84_ver}-Linux.x86_64.glibc2.35
    elif [ "${dbinstallmethod}" == "2" ]; then
      rm -rf percona-server-${percona84_ver} boost_${boostVersion2}
    fi
  else
