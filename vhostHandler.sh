...
ARG_NUM=$#
TEMP=$(getopt -o hql --long help,quiet,list,proxy,mphp_ver:,domain:,add,delete,del,httponly,selfsigned,letsencrypt,dnsapi -- "$@" 2>/dev/null)
...
while :; do
	...
	--mphp_ver)
		...
		mphp_flag=y
		shift 2
		[[ ! "${mphp_ver}" =~ ^5[3-6]$|^7[0-4]$ ]] && {
			echo "${CWARNING}mphp_ver input error! Please only input number 53~74${CEND}"
			unset mphp_ver mphp_flag
		}
		;;
	--domain)
		domain=$2
...
Choose_ENV() {
	if [ -e "${apache_install_dir}/bin/apachectl" ]; then
		[ "$(${apache_install_dir}/bin/apachectl -v | awk -F'.' /version/'{print $2}')" == '4' ] && {
			Apache_main_ver=24
...
Create_SSL() {
	if [ "${Domian_Mode}" == '2' ]; then
		echo
		read -e -p "Country Name (2 letter code) [BR]: " SELFSIGNEDSSL_C
		SELFSIGNEDSSL_C=${SELFSIGNEDSSL_C:-BR}
		...
		echo
		read -e -p "State or Province Name (full name) [Brasilia]: " SELFSIGNEDSSL_ST
		SELFSIGNEDSSL_ST=${SELFSIGNEDSSL_ST:-Brasilia}
		echo
		read -e -p "Locality Name (eg, city) [Brasilia]: " SELFSIGNEDSSL_L
		SELFSIGNEDSSL_L=${SELFSIGNEDSSL_L:-Brasilia}
		echo
		read -e -p "Organization Name (eg, company) [Example Inc.]: " SELFSIGNEDSSL_O
		SELFSIGNEDSSL_O=${SELFSIGNEDSSL_O:-"XMember."}
		echo
...
EOF
			...
			auth_file="$(tr </dev/urandom -dc A-Za-z0-9 | head -c8)".html
			auth_str='oneinstack'
			echo ${auth_str} >${vhostdir}/${auth_file}
			for D in ${domain} ${moredomainame}; do
				curl_str=$(curl --connect-timeout 30 -4 -s $D/${auth_file} 2>&1)
				[ "${curl_str}" != "${auth_str}" ] && {
					echo
					echo "${CFAILURE}Let's Encrypt Verify error! DNS problem: NXDOMAIN looking up A for ${D}${CEND}"
				}
			done
...
List_Vhost() {
	...
	if [ -n "${Domain_List}" ]; then 
		for D in ${Domain_List}; do echo ${D}; done
	else
		echo "\n"
	fi
}
